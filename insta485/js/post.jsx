...
export default function Post({ url }) {
  ...
  const handleLikeButton = () => {
    const targetUrl = post.lognameLikesThis
      ? post.likeUrl
      : `/api/v1/likes/?postid=${post.postid}`;
    // const newIsLiked = !post.lognameLikesThis;
    // const newNumLikes = post.numLikes + (newIsLiked ? 1 : -1);
    const methods = post.lognameLikesThis ? "DELETE" : "POST";
    fetch(targetUrl, { method: methods, credentials: "same-origin" })
      .then((response) => {
        if (!response.ok) throw Error(response.statusText);
  ...
  return (
    <div className="post">
      ...
      {/* < img src={post.imgUrl} alt="Post content" class="image2" /> */}
      <Postimg
        lognameLikesThis={post.lognameLikesThis}
        img={post.imgUrl}
